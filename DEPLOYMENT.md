...
**SQL Server (`login_otp_challenges`)**: if `flask db upgrade` failed with *The index 'ix_lotc_expires' is dependent on column 'expires_at' (5074)*, an autogenerated migration tried to **ALTER** a column while an index on that column still existed. The revision `migrations/versions/9003a3ff088a_.py` in this repo is fixed: it only drops the legacy indexes (`ix_lotc_expires`, `ix_lotc_user_id`, constraint `UQ_lotc_nonce` on Microsoft SQL Server) and creates the ORM index names. **No row data is changed.** Replace the broken generated file with the version from the repository, then run `flask db upgrade` again. If a failed run left a transaction open, use a new connection or retry; the migration is idempotent for existing target indexes.
